...
@idzcmd(command="backupdb", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def backup_now(_: Update, ctx: CallbackContext):
    cronjob.run(dispatcher=dispatcher)


@idzcmd(command="stopjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
def stop_jobs(update: Update, _: CallbackContext):
    print(j.stop())
    update.effective_message.reply_text("Scheduler has been shut down")


@idzcmd(command="startjobs", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
...
@idzcmd(command="purgebackups", filters=Filters.user(DEV_USERS) | Filters.user(OWNER_ID))
...
from aries import updater as u
